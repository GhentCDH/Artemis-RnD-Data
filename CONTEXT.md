...
- `static/`: manually edited viewer runtime content; not generated by the pipeline.
...
- `static/` is outside the preprocessing output contract; the viewer may read it directly at runtime without rerunning the pipeline.
- `build/index.json` exposes stable `layerId` values for `layers` and `renderLayers` so the viewer can join runtime-edited metadata from `static/`.
